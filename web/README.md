...
serves. **No bundler and no dependencies** — the site ships raw `.mjs`, and Node is needed only for
the asset build. **The page therefore needs the server reachable** to render the map; if it isn't, the
loading splash stays up with a *Maintenance Mode* notice (a deliberate hard dependency).

> A PixiJS renderer was vendored here in July 2026 and then removed: the target look
> ([`docs/terrain-3d.md`](../docs/terrain-3d.md)) needs a **3D** renderer, which Pixi is not. The
> dependency-free property above is the state that survived. See
> [`docs/pixi-migration-plan.md`](../docs/pixi-migration-plan.md) for the closed record, and
> [`docs/frontend-performance.md`](../docs/frontend-performance.md) for the measurement lessons —
> which is the part worth reading before optimising anything here.
